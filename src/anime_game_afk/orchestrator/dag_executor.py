...
Example::
    ...
    executor = DagExecutor(tool_registry, adapters, state, bus)
...
from anime_game_afk.orchestrator.post_actions import execute_post_action
...
class BaseAdapter(Protocol):
    ...
    Adapters translate the generic orchestrator commands into
    tool-specific process management.
    """
...
class DagExecutor:
    ...
    async def execute(
        ...
        Returns a :class:`RunResult` summarising every tool outcome.
        """
        self._cancelled = False
        ...
        # Post-actions run even on abort (e.g. kill leftover game processes)
    ...
    def cancel(self) -> None:
        ...
        Running processes are terminated at the next poll cycle.
        """
    ...
    async def _poll_until_done(
        ...
        while True:
            ...
            # Drain stdout/stderr without blocking
    ...
    async def _run_post_actions(
        ...
        """Execute the post-action from the plan."""
        logger.info(
