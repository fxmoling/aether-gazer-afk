...
Uses croniter for cron parsing; falls back to simple hour:minute
matching when croniter is not installed.

Example::

    scheduler = Scheduler(config_manager, executor_factory)
    scheduler.start()          # background thread begins polling
    scheduler.run_now("abc1")  # manual trigger
    scheduler.stop()
"""
...
if TYPE_CHECKING:
    ...
    from anime_game_afk.orchestrator.models import ScheduleEntry
...
_WAKE_TASK_PREFIX = "AnimeGameAFK_Wake_"
...
def _next_fire_time(cron_expr: str, base: datetime | None = None) -> datetime | None:
    ...
    Uses croniter when available; otherwise parses ``"M H * * *"`` style
    expressions and returns the next matching ``HH:MM`` today or tomorrow.
    """
...
def _cron_matches_now(cron_expr: str, now: datetime) -> bool:
    ...
    if _HAS_CRONITER:
        try:
            ...
            # Match if the previous fire time is within the same minute
            return (
                prev.year == now.year
...
def register_wake_task(schedule_id: str, cron_expr: str) -> bool:
    ...
    Creates a task named ``AnimeGameAFK_Wake_<schedule_id>`` that
    wakes the computer at the scheduled time.  Returns True on success.
    """
    ...
    # /SC DAILY /ST HH:MM /RL HIGHEST  with wake flag
    cmd = [
        "schtasks", "/Create",
        "/TN", task_name,
        "/TR", "cmd /c echo AnimeGameAFK wake trigger",
        "/SC", "DAILY",
        "/ST", time_str,
        "/F",  # force overwrite
        "/RL", "HIGHEST",
...
def _enable_wake_to_run(task_name: str) -> None:
    ...
    Uses ``schtasks /Change /ENABLE`` — the WakeToRun flag is not
    directly settable via command-line, but /RL HIGHEST already helps.
    """
    # schtasks CLI has limited wake support; best-effort
    try:
        subprocess.run(
...
class Scheduler:
    ...
    Runs in a daemon background thread, checks every 60 seconds if any
    :class:`ScheduleEntry`'s cron expression matches the current time.
    """
    ...
    def run_now(self, schedule_id: str) -> bool:
        ...
        Returns True if the schedule was found and execution started.
        """
        for entry in self._config.get_schedules():
            if entry.schedule_id == schedule_id:
    ...
    def _tick(self) -> None:
        """Single scheduler tick — check all schedules."""
        now = datetime.now()
    ...
    def _fire(self, entry: ScheduleEntry) -> None:
        """Execute a schedule entry in a background thread."""
        import asyncio
        ...
        thread = threading.Thread(
            target=_run,
            name=f"orch-run-{entry.schedule_id}",
            daemon=True,
        )
        thread.start()
