...
Example::
    ...
    action = PostAction(action_type=PostActionType.KILL_GAMES,
                        process_names=["YuanShen.exe", "GenshinImpact.exe"])
    success = await execute_post_action(action)
"""
...
from anime_game_afk.orchestrator.models import PostAction, PostActionType
...
# ---------------------------------------------------------------------------
# Individual post-action implementations
# ---------------------------------------------------------------------------
...
async def _kill_game_processes(action: PostAction) -> bool:
    ...
    Returns ``True`` if all processes were killed or were not running.
    """
    ...
    for name in action.process_names:
        ...
        try:
            ...
            if proc.returncode == 0:
                ...
                # "not found" variants are acceptable
...
async def _sleep_pc(action: PostAction) -> bool:
    ...
    Parameters: ``SetSuspendState(hibernate, force, disable_wake)``
    Using ``0,1,0`` → sleep (not hibernate), force, wake events enabled.
    """
    ...
    try:
        ...
        # rundll32 always returns 0, sleep may happen before we check
