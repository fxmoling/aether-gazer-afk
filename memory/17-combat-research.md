...
- **方案**: 灰度 `TM_CCOEFF_NORMED` 双信号 AND
  - Pause || (右上角): threshold=0.80, 战斗中 0.86-0.99
  - Dodge Space (右下角): threshold=0.40, 战斗中 0.43-0.93
- **非战斗 40 帧**: 0% 误报（22 个不同页面）
- **战斗 98 帧**: 100% 检出
- **红色闪避按钮**: dodge=0.54, 仍通过（阈值 0.40 有余量）
- **速度**: 1.2ms 中位数 (两信号合计)
- **关键设计决策**:
