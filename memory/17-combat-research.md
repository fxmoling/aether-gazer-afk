...
- **方案**: 灰度 `TM_CCOEFF_NORMED` **三信号 AND**
  - Signal 1: Pause || (右上角): threshold=0.80
  - Signal 2: Dodge Space (右下角): threshold=0.40
  - Signal 3: Skill-region std (J/Tab/U/I/O 区域): threshold=40.0
- **三信号设计原因**:
  - 多维变量探索模式有 Pause + Dodge 但无技能按钮 → 双信号会误判
  - Signal 3 用技能区域灰度标准差区分: 战斗 std=45-73, 探索 std=5-32, 菜单不过模板
- **非战斗测试**: 23 张截图 (4 战斗 + 11 探索 + 8 菜单), **0 错误**
- **实战测试 (黑区净化, 50 帧)**:
  - 战斗中: 41/46 帧正确检出 (**89%**)
  - Pause: 0.807-0.980, Dodge: 0.669-0.886, Std: 45.2-73.4
  - 5 帧假阴性 — 技能特效/全屏闪光暂时遮挡 Pause 图标 (P=0.52-0.78)
  - 战斗结束: 4/4 帧正确拒绝 (P=0.21, std=10-16)
  - **0 假阳性** (最重要指标)
- **假阴性分析**: 全屏特效 (大招动画) 短暂遮挡 Pause 图标，0.5-2s 后恢复。对自动战斗无影响（重试即可）
- **速度**: ~1ms (模板匹配) + std 计算可忽略
- **关键设计决策**:
  - 灰度转换消除背景颜色干扰（技能特效、火焰等）
  - CCOEFF_NORMED 减均值后比较，只看局部对比度图案
  - Dodge 阈值从 0.80 降到 0.40（实测闪避按钮受特效影响大，但非战斗时最高仅 0.06）
  - Skill-region std 阈值 40.0，战斗最低 45.2 vs 探索最高 32.4，间隔 12.8 点
- 假阴性无害（秒级重试），假阳性有害（非战斗场景按键）
