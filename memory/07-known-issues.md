...
- **创建**: 2026-04-04
- **上次更新**: 2026-04-23 — 添加自动更新 TODO

## 6. 自动更新机制缺失 (TODO)

**现状**: updater.py 只检测 GitHub 新版本并展示提示横幅，用户需要手动下载 zip 并覆盖安装。
**对比**: ok-ww 使用 PyAppify 方案，EXE 是固定壳，每次启动自动 git pull 最新源码，用户完全无感更新。
**我们的方案**: PyInstaller 把源码冻结进 EXE，更新 = 替换整个 262MB 文件夹。

**可选方案**:
1. **PyInstaller + 自动下载替换** — 下载 zip → 解压到临时目录 → 关闭自身 → 批处理覆盖旧文件 → 重新启动（hacky 但常见）
2. **换 PyAppify** — 重构打包方式，代码通过 git 自动更新，用户无感（大改动，需评估 DLL/MaaFw 兼容性）

**优先级**: 低 — 当前手动更新可接受，用户量增长后再考虑
